...
object TabletoWFT {
  ...
  def headerPathstoDataCells(table:Table):Option[WFT]={
    cellClassify(table) match{
      case Some(wft) =>Some(
        wft.copy(dcs=wft.dcs.map(dc=>dc.copy(rowpath=getRowPath(dc,wft),
  ...
  def getColPath(dc:C,wft:WFT):Seq[B]= wft.chcs.filter(x=>x.startRow<dc.startRow && x.startCol<=dc.startCol
    && wft.chcs.filter(y=>y.startCol<=dc.startCol&&y.startRow==x.startRow&&y.startCol>x.startCol).isEmpty
  ).sortWith(_.startRow<_.startRow)

  def getRowPath(dc:C,wft:WFT):Seq[A]= wft.rhcs.filter(x=>x.startCol<dc.startCol && x.startRow<=dc.startRow
    && wft.rhcs.filter(y=>y.startRow<=dc.startRow&&y.startCol==x.startCol&&y.startRow>x.startRow).isEmpty
  ).sortWith(_.startCol<_.startCol)
